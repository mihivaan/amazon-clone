...
const Checkout = () => {
  ...
  const subtotal = useSelector((state) => state.cart.products.reduce((subtotal, product) => subtotal + (product.price * 102 * product.quantity), 0));
  const dispatch = useDispatch();

  return (
    <div className='h-screen bg-amazonclone-background min-w-[1000px]'>
      <div className='min-w-[1000px] max-w-[1500px] m-auto pt-8'>
        <div className='grid grid-cols-8 gap-10'>
          ...
          <div className='col-span-6 bg-white'>
                ...
                return (
                  <div key={product.id}>
                    <div className='grid grid-cols-12 divide-y divide-gray-400 mr-4'>
                      ...
                      <div className="col-span-2">
                        <div className='text-lg xl:text-xl mt-2 mr-4 font-semibold'>{IN_CURRENCY.format(product.price * 102)}</div>
                      </div>
                    </div>
                  </div>
                )
            ...
            <div className='text-lg xl:text-xl text-right mb-4 mr-4'>Subtotal ({itemsNumber} items): <span className='font-semibold'>{IN_CURRENCY.format(subtotal)}</span></div>
          </div>
          {/* Checkout */}
          <div className="col-span-2 bg-white rounded h-[250px] p-7">
            ...
            <div className='text-base xl:text-lg mb-4'>Subtotal ({itemsNumber} items): <span className='font-semibold'>{IN_CURRENCY.format(subtotal)}</span></div>
